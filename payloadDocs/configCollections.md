...
Specify which fields should be selected always, regardless of the select query which can be useful that the field exists for access control / hooks. More details.
...
Function to customize document links in the List View. Return null to disable linking, or a string for custom URLs. More details.
...
The current view context ('list', 'trash', etc.) where the link is being generated
